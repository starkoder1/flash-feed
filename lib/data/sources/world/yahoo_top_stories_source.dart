...
class YahooTopNewsSource {
  ...
  Future<List<NewsItem>> fetchNews() async {
    ...
    try {
      ...
      debugPrint('❌ Error in YahooTopNewsSource.fetchNews: $e');
  ...
  DateTime _parseDate(String? dateStr) {
    if (dateStr == null || dateStr.isEmpty) {
      debugPrint(
        '⚠️ Date string was null/empty for Yahoo News. Using DateTime.now().',
      );
      return DateTime.now();
    }
    try {
      ...
      try {
        ...
        try {
          ...
          debugPrint(
            '⚠️ Failed to parse date "$dateStr" for Yahoo News. Using DateTime.now().',
          );
          return DateTime.now();
        }
      }
